...
async def trigger_posting(slot: str = "morning"):
    """Scheduled task for daily 09:00 & 21:00 posts."""
    ...
    try:
        ...
        if success:
            ...
            logger.error(f"Failed to publish {slot} post.")
...
def setup_scheduler(app_instance: Application) -> AsyncIOScheduler:
    """Configures APScheduler for 09:00 & 21:00 daily posts, Friday 07:45 special, and Sunday report."""
    ...
    logger.info(f"Scheduler configured for {POST_TIMEZONE} timezone.")
    return scheduler
